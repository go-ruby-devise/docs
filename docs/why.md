...
`go-ruby-devise/devise` reimplements Ruby's `devise` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-devise/devise` directly, with no Ruby runtime;
...
Because `devise` is pure Go with cgo disabled, it also compiles to **WebAssembly**
